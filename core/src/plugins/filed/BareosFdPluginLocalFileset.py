...
#            return bRCs["bRC_OK"]
    ...
    def end_restore_file(self, context):
        bareosfd.DebugMessage(
                context, 100, "end_restore_file() entry point in Python called FNAME: %s\n" %self.FNAME
        )
        bareosfd.DebugMessage(
            context,
            150,
            "end_restore_file set file attributes " + self.FNAME + " with stat " + str(self.statp[self.FNAME]) + "\n",
        )
        try:
            os.chown(self.FNAME, self.statp[self.FNAME].uid, self.statp[self.FNAME].gid)
            os.chmod(self.FNAME, self.statp[self.FNAME].mode)
            os.utime(self.FNAME, (self.statp[self.FNAME].atime, self.statp[self.FNAME].mtime))
            newStat = os.stat(self.FNAME)
            bareosfd.DebugMessage(
                context,
                150,
                "Verified file attributes " + self.FNAME + " with stat " + str(newStat) + "\n",
            )
